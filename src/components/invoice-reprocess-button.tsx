...
import { PaperclipIcon, RefreshCwIcon, XIcon } from "lucide-react";
...
type InvoiceReprocessButtonProps = {
  invoiceId: string;
...
export function InvoiceReprocessButton({
  ...
  function openDialog() {
    setSelectedIds(supportedAttachments.map((attachment) => attachment.id));
    setFile(null);
    setError(null);
    setShowAll(false);
    setOpen(true);
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={(next) => !next && !loading && setOpen(false)}>
        <DialogContent>
          <DialogHeader>
