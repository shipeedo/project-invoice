...
async function getSupplierNoteCounts(organizationId: string) {
  const rows = await db
    .select({ supplierId: notes.supplierId, noteCount: count() })
    .from(notes)
    .innerJoin(suppliers, eq(notes.supplierId, suppliers.id))
    .where(eq(suppliers.organizationId, organizationId))
    .groupBy(notes.supplierId);

  return new Map(rows.map((row) => [row.supplierId, row.noteCount]));
}
...
export default async function SuppliersPage() {
  ...
  const [rows, suggestions, stats, noteCounts, navCounts] = await Promise.all([
    ...
    getSupplierNoteCounts(organizationId),
        ...
        <SuppliersManager
          initialSuppliers={rows.map((supplier) => {
            ...
            return {
              ...
              noteCount: noteCounts.get(supplier.id) ?? 0,
            };
          })}
